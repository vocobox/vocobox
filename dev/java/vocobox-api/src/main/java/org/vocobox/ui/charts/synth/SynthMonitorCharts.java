...
public class SynthMonitorCharts extends Timer implements SynthMonitor {
    ...
    public void makePitchChart() {
        ...
        // should share the same space transform for all? -> on add drawable via
        // scene graph, add current transform
    ...
    public void setAutoFit() {
        if (pitchChart != null)
            pitchChart.getView().setBoundMode(ViewBoundMode.AUTO_FIT);
        if (ampliChart != null)
            ampliChart.getView().setBoundMode(ViewBoundMode.AUTO_FIT);
        if (confidenceChart != null)
            confidenceChart.getView().setBoundMode(ViewBoundMode.AUTO_FIT);
    }
